...
public class Frame extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        txtResultado.setText("procedure Saludo();\nbegin\n \nend;\n\n");
    ...
    private void btnAnalizarSinActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAnalizarSinActionPerformed
        // Obtener el texto de la interfaz de usuario
    String ST = txtResultado.getText().trim();  // Eliminar espacios en blanco al principio y al final del texto
    
    // Verificar si el texto está vacío
    if (ST.isEmpty()) {
        // Si el texto está vacío, mostrar un mensaje de error en la interfaz
        txtAnalizarSin.setText("Error: No se proporcionó ningún código para analizar.");
        txtAnalizarSin.setForeground(Color.red);
        return;  // Salir del método
    }
    // Crear el analizador sintáctico y pasar el texto
    Sintax s = new Sintax(new Dev.LexicoCup(new StringReader(ST)));
    try {
        // Intentar realizar el análisis sintáctico
        s.parse();
        
        // Si el análisis se completa sin errores, mostrar un mensaje de éxito en la interfaz
        txtAnalizarSin.setText("Análisis realizado correctamente");
        txtAnalizarSin.setForeground(new Color(25, 111, 61));
    } catch (Exception ex) {
        // Si se produce un error durante el análisis, obtener el símbolo asociado al error
        Symbol sym = s.getS();
        
        // Verificar si se encontró un símbolo de error
        if (sym != null) {
            // Si se encontró un símbolo de error, mostrar información sobre el error en la interfaz
            txtAnalizarSin.setText("Error de sintaxis. Línea: " + (sym.right + 1) + " Columna: " + (sym.left + 1) + ", Texto: \"" + sym.value + "\"");
        } else {
            // Si no se encontró un símbolo de error, mostrar un mensaje de error genérico en la interfaz
            txtAnalizarSin.setText("Error de sintaxis. No se pudo identificar el error.");
        }
        // Establecer el color del texto en rojo para indicar un error
        txtAnalizarSin.setForeground(Color.red);
    }
